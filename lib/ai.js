...
// ───────── CUSTOM PERSONAS (계정 동기화) ─────────
// ai_personas/{uid}/{id} = { name, tagline, description, greeting, icon, accent, instructions, custom, createdAt }
export async function listPersonas(uid) {
  if (!uid) return [];
  try {
    const snap = await get(ref(db, `ai_personas/${uid}`));
    if (!snap.exists()) return [];
    return Object.entries(snap.val()).map(([id, v]) => ({ id, ...v }));
  } catch (e) {
    console.warn('[AltroAi] listPersonas 실패 (규칙 미게시 가능성):', e?.message || e);
    return [];
  }
}

export async function savePersona(uid, persona) {
  if (!uid || !persona?.id) return;
  const { id, ...data } = persona;
  await set(ref(db, `ai_personas/${uid}/${id}`), { ...data, createdAt: data.createdAt || new Date().toISOString() });
}

export async function deletePersonaRemote(uid, id) {
  if (!uid || !id) return;
  await remove(ref(db, `ai_personas/${uid}/${id}`));
}
